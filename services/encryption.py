...
Encryption keys are derived from the user's password (PBKDF2-HMAC-SHA256, OWASP
2023 iterations). Each loan has its own data encryption key (DEK), which is
wrapped with the user's password and optionally with a recovery phrase. The
server never sees a key it can use without the user's password.
"""
